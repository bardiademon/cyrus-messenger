...
public final class RestNewManager
{
             ...
             @RequestBody RequestNewManager request)
    {
        ...
        CBSIL both = CBSIL.Both (request , req , res , codeLogin , userLoginService , Domain.RNChat.RNGroups.Security.RN_SECURITY_NEW_MANAGER , SubmitRequestType.new_manager);
        if (both.isOk ())
        {
            ...
            if (request != null)
            {
                        ...
                        if (idUser.isValid ())
                        {
                            ...
                            if (mainAccountUser != null)
                            {
                                if (mainAccount.getId () != mainAccountUser.getId ())
                                {
                                    ...
                                    if (isJoined.is ())
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            l.n (ToJson.To (request) , Domain.RNChat.RNGroups.Security.RN_SECURITY_NEW_MANAGER , mainAccount , answerToClient , Thread.currentThread ().getStackTrace () , new Exception (ValAnswer.this_user_is_manager.name ()) , cookieJson);
                                    ...
                                    else
                                    {
                                        ...
                                        answerToClient.setReqRes (req , res);
                                        l.n (ToJson.To (request) , Domain.RNChat.RNGroups.Security.RN_SECURITY_NEW_MANAGER , mainAccount , answerToClient , Thread.currentThread ().getStackTrace () , new Exception (ValAnswer.this_user_is_not_member_of_group.name ()) , cookieJson);
                                        r.n (mainAccount , SubmitRequestType.group_members , true);
                                    }
                                ...
                                else
                                {
                                    ...
                                    answerToClient.setReqRes (req , res);
                                    l.n (ToJson.To (request) , Domain.RNChat.RNGroups.Security.RN_SECURITY_NEW_MANAGER , mainAccount , answerToClient , Thread.currentThread ().getStackTrace () , new Exception (ValAnswer.this_id_belongs_to_you.name ()) , cookieJson);
                                    r.n (mainAccount , SubmitRequestType.group_members , true);
                                }
                            }
                            else
                            {
                                ...
                                answerToClient.setReqRes (req , res);
                                l.n (ToJson.To (request) , Domain.RNChat.RNGroups.Security.RN_SECURITY_NEW_MANAGER , mainAccount , answerToClient , Thread.currentThread ().getStackTrace () , new Exception (ValAnswer.user_not_found.name ()) , cookieJson);
                                r.n (mainAccount , SubmitRequestType.group_members , true);
                            }
                        }
                        else
                ...
                {
                    answerToClient.setReqRes (req , res);
                    l.n (ToJson.To (request) , Domain.RNChat.RNGroups.Security.RN_SECURITY_NEW_MANAGER , mainAccount , answerToClient , Thread.currentThread ().getStackTrace () , new Exception (ValAnswer.invalid_request.name ()) , cookieJson);
                    r.n (mainAccount , SubmitRequestType.group_members , true);
                }
            }
            else
            {
                ...
                answerToClient.setReqRes (req , res);
                l.n (null , Domain.RNChat.RNGroups.Security.RN_SECURITY_NEW_MANAGER , mainAccount , answerToClient , Thread.currentThread ().getStackTrace () , new Exception (ValAnswer.id_group_invalid.name ()) , cookieJson);
                r.n (mainAccount , SubmitRequestType.group_members , true);
            }
        }
