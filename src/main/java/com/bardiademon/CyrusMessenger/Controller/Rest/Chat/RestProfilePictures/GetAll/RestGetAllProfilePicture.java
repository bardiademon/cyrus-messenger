...
@RestController
@RequestMapping (value = Domain.RNChat.RNProfilePicture.RN_PROFILE_PICTURE_GET_ALL, method = RequestMethod.POST)
public final class RestGetAllProfilePicture
{
             ...
             HttpServletResponse res , HttpServletRequest req , @RequestBody RequestGetAllProfilePicture request)
    {
        ...
        {
            IdUsername idUsername = new IdUsername (mainAccountService , request.getId () , request.getUsername ());
            if (idUsername.isValid ())
            {
                MainAccount mainAccountRequested = checkLogin.getVCodeLogin ().getMainAccount ();
                CheckUserAccessLevel checkUserAccessLevel = new CheckUserAccessLevel (mainAccountRequested , idUsername.getMainAccount () , mainAccountService);

                ToJson.CreateClass createClass = new ToJson.CreateClass ();
                createClass.put ("id_user_get_cover" , idUsername.getIdUser ());
                ...
                if (checkUserAccessLevel.hasAccessProfile (CheckUserAccessLevel.CheckProfile.cover))
                {
                    ...
                    {
                        String rnProfilePictureGetOne = Domain.RNChat.RNProfilePicture.RN_PROFILE_PICTURE_GET_ONE;
                    ...
                    l.n (ToJson.To (request) , Domain.RNChat.RNProfilePicture.RN_PROFILE_PICTURE_GET_ALL , mainAccountRequested , answerToClient , Thread.currentThread ().getStackTrace () , null , createClass.toJson ());
                }
                else
                {
                    answerToClient = AnswerToClient.New (HttpServletResponse.SC_UNAUTHORIZED);
                    answerToClient.setReqRes (req , res);
                    l.n (ToJson.To (request) , Domain.RNChat.RNProfilePicture.RN_PROFILE_PICTURE_GET_ALL , mainAccountRequested , answerToClient , Thread.currentThread ().getStackTrace () , new Exception ("Status code:" + HttpServletResponse.SC_UNAUTHORIZED) , createClass.toJson ());
                }
            }
            else
            {
                answerToClient = idUsername.getAnswerToClient ();
                answerToClient.setReqRes (req , res);
                l.n (ToJson.To (request) , Domain.RNChat.RNProfilePicture.RN_PROFILE_PICTURE_GET_ALL , null , answerToClient , Thread.currentThread ().getStackTrace () , new Exception ("Error from class IdUsername") , null);
            }
        }
        else
        {
            ...
            answerToClient.setReqRes (req , res);
            l.n (ToJson.To (request) , Domain.RNChat.RNProfilePicture.RN_PROFILE_PICTURE_GET_ALL , null , answerToClient , Thread.currentThread ().getStackTrace () , new Exception ("not login") , null);
        }
